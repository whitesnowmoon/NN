...
public:
	...
	Matrix(int Matrix_row, int Matrix_column, Type_* const Matrix_data);
	...
	void fun_handle(Type_(*fun)(Type_));//每个数据函数处理
	void rotate();				//转置矩阵	
	void Uppertriangle();//该函数不完善，要求矩阵必须满秩
	void Lowertriangle();//该函数不完善，要求矩阵必须满秩
	Matrix<Type_> hconcat(const Matrix& t);//矩阵拼接 横向
	Matrix<Type_> vconcat(const Matrix& t);//矩阵拼接 竖向
	Matrix<Type_> Diagonal();//对角化
	Matrix<Type_> solution(const Matrix& resulted);//高斯消元法求解 该函数不完善，要求矩阵必须满秩
	Matrix<Type_> solution();//该函数不完善，要求矩阵必须满秩
...
template<typename Type_>
inline Matrix<Type_>::Matrix(int Matrix_row, int Matrix_column, Type_* const Matrix_data) {
	this->Matrix_row = Matrix_row;
	this->Matrix_column = Matrix_column;
	this->Matrix_data = new Type_ * [Matrix_row];
	for (int i = 0; i < Matrix_row; i++)
	{
		this->Matrix_data[i] = new Type_[Matrix_column];
	}
	for (int i = 0; i < Matrix_row; i++)
	{
		for (int j = 0; j < Matrix_column; j++)
		{
			this->Matrix_data[i][j] = Matrix_data[(i * Matrix_column) + j];
		}
	}
}
...
template<typename Type_>
Matrix<Type_> Matrix<Type_>::Diagonal() {
...
template<typename Type_>
Matrix<Type_> Matrix<Type_>::solution(const Matrix& resulted) {
	Matrix<Type_>* newone = new Matrix<Type_>(*this);
	*newone=newone->hconcat(resulted);
	newone->Uppertriangle();
	newone->Lowertriangle();
	std::vector<Type_> datam;
	for (size_t i = 0; i < newone->Matrix_row; i++)
	{
		datam.emplace_back(newone->Matrix_data[i][newone->Matrix_column - 1]);
	}
	delete newone;
	return Matrix<Type_>(datam);
}

template<typename Type_>
inline Matrix<Type_> Matrix<Type_>::solution() {
	Matrix<Type_>* newone = new Matrix<Type_>(*this);
	newone->Uppertriangle();
	newone->Lowertriangle();
	std::vector<Type_> datam;
...
template<typename Type_>
void Matrix<Type_>::Uppertriangle()//化上三角
{
	Type_ digx;
	for (unsigned int row = 0; row < (this->Matrix_row - 1); row++)
	{
		Type_* tempm;
		for (unsigned int i = row + 1; i < this->Matrix_row; i++)
		{
			if (fabs(this->Matrix_data[row][row]) < fabs(this->Matrix_data[i][row]))
			{
				tempm = this->Matrix_data[row];
				this->Matrix_data[row] = this->Matrix_data[i];
				this->Matrix_data[i] = tempm;
			}
		}
		digx = this->Matrix_data[row][row];
		for (unsigned int residue = row; residue < this->Matrix_column; residue++)
		{

			this->Matrix_data[row][residue] = this->Matrix_data[row][residue] / digx;
		}
		for (int i = row + 1; i < this->Matrix_row; i++)
		{
			digx = this->Matrix_data[i][row];
			for (int a = row; a < this->Matrix_column; a++)
			{
				this->Matrix_data[i][a] = this->Matrix_data[i][a] - this->Matrix_data[row][a] * digx;
			}
...
template<typename Type_>
void Matrix<Type_>::Lowertriangle()//化下三角
{
	Type_ digx;
	for (int i = this->Matrix_row-1; i > 0; i--)
	{
		digx = this->Matrix_data[i][i];
		for (int a = i; a < this->Matrix_column; a++)
		{
				this->Matrix_data[i][a] = this->Matrix_data[i][a] / digx;
		}
		for (int j = i - 1; j >= 0; j--)
		{
			digx = this->Matrix_data[j][i];
			for (int a = j; a < this->Matrix_column; a++)
			{
				this->Matrix_data[j][a] = this->Matrix_data[j][a] - this->Matrix_data[i][a] * digx;
			}
		}
	}
}
